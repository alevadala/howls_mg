...
bs = -0.5 # Bias
mu_j = 0 # Order of the ordinary Bessel function, not the spherical one
conv_factor = 206265/60 # Conversion factor from radians to arcmins
correction = np.sqrt(np.pi/2)*conv_factor**1.275 # Total correction to account for conversion and spherical Bessel
# correction = np.sqrt(np.pi/2)*conv_factor**1.295
...
# Set the Limber correction on/off
correct_limber = 1
...
for cosmo in cosmos:
    ...
    for zs in zs_values:
        ...
        # # Loading measured 2PCF DVs
        # for map in np.linspace(100,255,10):
        #     if cosmo == 'lcdm':
        #         two_pt = np.loadtxt(dv_path+f'{round(map)}_LCDM_DUSTGRAIN_convergence_true_LCDM_z_{zs}_kappa_2pcf.txt',usecols=3)
        #         xr = np.loadtxt(dv_path+f'{round(map)}_LCDM_DUSTGRAIN_convergence_true_LCDM_z_{zs}_kappa_2pcf.txt',usecols=1)
        #     else:
        #         two_pt = np.loadtxt(dv_path+f'{round(map)}_LCDM_DUSTGRAIN_convergence_true_'+ plot_label.replace(' ','_') + f'_z_{zs}_kappa_2pcf.txt',usecols=3)
        #         xr = np.loadtxt(dv_path+f'{round(map)}_LCDM_DUSTGRAIN_convergence_true_'+ plot_label.replace(' ','_') + f'_z_{zs}_kappa_2pcf.txt',usecols=1)
            
        #     # Plotting and saving measurements/theory comparison for given maps
        #     plt.figure(figsize=(8,6))
        #     plt.scatter(xr,two_pt*xr**2,label='Measured',color='k')
        #     plt.plot(theta,xi_fft*theta**2,label='Theory',color='r')
        #     plt.xscale('log')
        #     plt.yscale('log')
        #     plt.xlim(twopt_min,twopt_max)
        #     plt.title(rf'$\kappa$ - 2PCF, {plot_label}, $z_s$={zs}, {round(map)}',fontsize=16)
        #     plt.xlabel(r'$\theta$ (arcmin)',fontsize=14)
        #     plt.ylabel(r'$\theta^{2}\,\xi \, (\theta)$',fontsize=14)
        #     plt.xticks([0.1,1,10,100,400],[0.1,1,10,100,400])
        #     plt.legend()
        #     plt.savefig(maps_out+f'{round(map)}_{cosmo}_{zs}.png', dpi=300) 
        #     plt.clf()
        #     plt.close('all')
        ...
        plt.errorbar(xr,two_pt*xr**2,yerr=err*xr**2*2e5,color='k')
        ...
        # plt.figure(figsize=(8,6))
        # plt.scatter(xr,two_pt,label='Measured',color='k')
        # plt.plot(theta,xi_fft,label='Theory',color='r')
        # plt.xscale('log')
        # plt.yscale('log')
        # plt.xlim(twopt_min,twopt_max)
        # plt.title(rf'$\kappa$ - 2PCF, {plot_label}, Mean values, $z_s$={zs}',fontsize=16)
